package net.prangellplays.pstranslate.event;
...
import net.prangellplays.pstranslate.PSTranslatorClient;
import net.prangellplays.pstranslate.config.ConfigUI;
import net.prangellplays.pstranslate.unit.MicrophoneHandler;
import net.prangellplays.pstranslate.unit.SpeechRecognizer;
...
public class EventHandler {
            ...
            try {
                if (speechRecognizer == null) {         // wait 10 seconds and try to initialize the speech recognizer again
                    ...
                    if (!tmp.equals("") && !tmp.equals(lastResult) &&
                            PSTranslatorClient.vKeyBinding.isPressed()) {   // Read audio data from the microphone and send it to the speech recognizer for recognition
                ...
                PSTranslator.LOGGER.error(e.getMessage());
            }
        }
    }

    private static void handelClientStartEvent(MinecraftClient client) {     // when the client launch
        PSTranslator.LOGGER.info("Loading acoustic model from " + ConfigUI.acousticModelPath + "   ..."); // Log the path of the acoustic model
        try {                                  // Initialize the speech recognizer
            speechRecognizer = new SpeechRecognizer(new Model(ConfigUI.acousticModelPath), ConfigUI.sampleRate);
            PSTranslator.LOGGER.info("Acoustic model loaded successfully!");
        }catch (Exception e1) {
            PSTranslator.LOGGER.error(e1.getMessage());
        }
        try {                                   // Initialize the microphone handler, single channel, 16 bits per sample, signed, little endian
            ...
            PSTranslator.LOGGER.info("Microphone handler initialized successfully!");
        } catch (Exception e2) {
            PSTranslator.LOGGER.error(e2.getMessage());
        }
        if (ConfigUI.encoding_repair) {         // If the encoding repair function is enabled, log a warning
            PSTranslator.LOGGER.warn(
    ...
    private static void handleEndClientTickEvent(MinecraftClient client) {     // When the client ticks, check if the user presses the key V
        if (client.player!=null &&                                             // If the player is not null
                PSTranslatorClient.vKeyBinding.isPressed() &&           // If the user presses the key V
    ...
    private static void handleStartClientTickEvent(MinecraftClient client) {  // handle another client tick event to notify the user that the speech recognition is in progress and the game is not frozen
        if (client.player!=null && PSTranslatorClient.vKeyBinding.isPressed()) {  // If the user presses the key V
